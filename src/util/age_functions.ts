...
function extractAgeRange(query: string) {
  ...
  const minMatch = query.match(/(?:above|over|older than|at least|minimum of?|from)\s+(\d+)/i);
  if (minMatch && minMatch[1]) result.min = parseInt(minMatch[1]);

  const maxMatch = query.match(/(?:below|under|younger than|at most|maximum of?|up to)\s+(\d+)/i);
...
function extractAgeGroup(
  tokens: string[],
  explicit: { min?: number; max?: number; age_group?: string },
) {
